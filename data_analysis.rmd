...
Perform t-test to examine the difference between genders

```{r}
t_result <- t.test(as.numeric(Class.ASD) ~ as.factor(gender), dt)
```

```{r}
print(t_result)
```
The t-test results indicate a statistically significant difference in the number of autism diagnoses between genders (p-value ~ 0.033). 
...
Perform t-test:
```{r}
t_result <- t.test(as.numeric(result) ~ as.factor(gender), dt)
```
...
```{r}
t_result <- t.test(as.numeric(result) ~ as.factor(austim), data = dt)
```

```{r}
print(t_result)
```
The results of the t-test indicate statistical significance of the family history of autism (marked as ***).
...
Finally, a strong association was observed between family history of autism and higher AQ-10 results, with the t-test confirming this factor as highly significant.
